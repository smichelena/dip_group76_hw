...
{   
    ...
    cout << "Dip3::circShift() seems to be correct!" << endl;
    return true;
}
...
bool test_usm(void){
    ...
    Mat output_freq = usm(input, FM_FREQUENCY_CONVOLUTION, 5, 1.0f, 5.0f);
    Mat output_space = usm(input, FM_SPATIAL_CONVOLUTION, 5, 1.0f, 5.0f);
    Mat output_sep = usm(input, FM_SEPERABLE_FILTER, 5, 1.0f, 5.0f);

    // std::cout << "frequency" << "\n";
    // print_matrix(output_freq);

    // std::cout << "spatial" << "\n";
    // print_matrix(output_space);

    // std::cout << "separable" << "\n";
    // print_matrix(output_sep);
