...
cv::Mat_<cv::Vec3b> processColorImage(const cv::Mat_<cv::Vec3b> &src, dip3::FilterMode filterMode, int size, float thresh, float scale)
{
    ...
    //planes[1] = usm(planes[1], filterMode, size, thresh, scale);
    //planes[0] = usm(planes[0], filterMode, size, thresh, scale);
...
*/
    ...
    {
        cv::Mat output = processColorImage(imgIn, (dip3::FilterMode)i, 5, 1.0f, 15.0f);
